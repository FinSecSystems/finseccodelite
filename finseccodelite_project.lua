--
-- Name:        finseccodelite/finseccodelite_project.lua
...
--
	...
	finseccodelite.project = {}
	local m = finseccodelite.project


	function finseccodelite.getLinks(cfg)
		-- System libraries are undecorated, add the required extension
		return config.getlinks(cfg, "system", "fullpath")
	end

	function finseccodelite.getSiblingLinks(cfg)
		...
		tree.traverse(tr, {
			...
			onleaf = function(node, depth)
				...
				for cfg in project.eachconfig(prj) do
					local cfgname = finseccodelite.cfgname(cfg)
...
--		_p(1, '<Dependencies/>')
		...
		for cfg in project.eachconfig(prj) do
			cfgname = finseccodelite.cfgname(cfg)
